(function initServiceWorker(root) {
  ...
  const contentMessenger = namespace.backgroundContentMessenger.create(api, {
    manualEditableMessageType: messageTypes.manualTranslateEditable
  });
  ...
  function subtitleTranslationCacheKey(text, previousText, nextText, sourceLanguage, targetLanguage, providerSignature, contextStyle, subtitleContext, autoCorrectSubtitleText, pageKeywords) {
    return [
      ...
      String(subtitleContext || "").replace(/\s+/g, " ").trim().slice(0, 600),
      autoCorrectSubtitleText ? "asr-correct" : "no-asr-correct",
      autoCorrectSubtitleText ? normalizeSubtitlePageKeywords(pageKeywords).slice(0, 500) : "",
      providerSignature || "",
      String(previousText || "").replace(/\s+/g, " ").trim().slice(0, 500),
      String(nextText || "").replace(/\s+/g, " ").trim().slice(0, 500),
      text || ""
  ...
  function subtitleTopicContextCacheKey(sampleText, sourceLanguage, targetLanguage, providerSignature, title, pageKeywords) {
    return [
      ...
      normalizeSubtitlePageKeywords(pageKeywords).slice(0, 500),
  ...
  function isYouTubeSubtitleSourceKind(kind) {
    ...
    return value === "youtube" || value === "youtube-dom";
  }

  function isAutoGeneratedYouTubeSubtitleRequest(message) {
    if (!isYouTubeSubtitleSourceKind(message && message.subtitleSourceKind)) {
      return false;
    }
    if (message && message.subtitleAutoGenerated === true) {
      return true;
    }
    const track = {
      kind: message && message.subtitleTrackKind,
      vssId: message && message.subtitleTrackVssId,
      name: message && message.subtitleTrackName
    };
    return !!(namespace.videoSubtitleUtils
      && typeof namespace.videoSubtitleUtils.isYouTubeAutoGeneratedTrack === "function"
      && namespace.videoSubtitleUtils.isYouTubeAutoGeneratedTrack(track));
  }

  function shouldAutoCorrectSubtitleText(message, settings) {
    return !!(message
      && message.autoCorrectSubtitleText === true
      && settings
      && settings.videoBilingualSubtitlesAutoCorrectAsr === true
      && isAutoGeneratedYouTubeSubtitleRequest(message));
  }
  ...
  function normalizeSubtitlePageKeywords(text) {
    const seen = new Set();
    return String(text || "")
      .replace(/\u00a0/g, " ")
      .split(/[,;|]/)
  ...
  function buildSubtitleTopicContextInput(message) {
    ...
    const pageKeywords = normalizeSubtitlePageKeywords(message.pageKeywords);
    const sampleText = normalizeSubtitleTopicSampleText(message.sampleText);
    return [
      title ? `Video title: ${title}` : "",
      url ? `Page URL: ${url}` : "",
      pageKeywords ? `Page keywords: ${pageKeywords}` : "",
  ...
  async function handleSenseSubtitleTopicContext(message) {
    ...
    try {
      ...
      const pageKeywords = normalizeSubtitlePageKeywords(message.pageKeywords);
      const key = subtitleTopicContextCacheKey(sampleText, sourceLanguage, targetLanguage, providerSignature, title, pageKeywords);
  ...
  async function translateSubtitleTextPart(partText, details) {
    const request = {
      ...
      autoCorrectSubtitleText: details.autoCorrectSubtitleText === true,
      pageKeywords: details.autoCorrectSubtitleText === true ? details.pageKeywords : "",
    ...
    const key = subtitleTranslationCacheKey(
      ...
      request.autoCorrectSubtitleText,
  ...
  async function handleTranslateSubtitleBatch(message) {
    ...
    const autoCorrectSubtitleText = shouldAutoCorrectSubtitleText(message, settings);
    const pageKeywords = autoCorrectSubtitleText ? normalizeSubtitlePageKeywords(message.pageKeywords) : "";
  ...
  api.contextMenus.onClicked(async (info, tab) => {
    ...
    const frameOptions = contentMessenger.getFrameOptions(info.frameId);
    ...
    try {
      await contentMessenger.sendContextMenuMessage(tab.id, message, frameOptions);
    } catch (error) {
